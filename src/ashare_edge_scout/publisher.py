...
def _write_discovery_csv(path: Path, results: Sequence[EdgeScoutResult]) -> None:
    ...
    fields = [
        ...
        "candle_upper_shadow_pct", "candle_long_upper_shadow_risk", "candle_bullish_reversal",
...
def _write_daily_research_watchlist_csv(path: Path, results: Sequence[EdgeScoutResult]) -> None:
    ...
    fields = [
        ...
        "cnstock_base_score", "edge_score", "start_signal_count", "start_signals",
        "futu_risk_codes", "pct_chg", "ret_5d", "amount_cny", "turn", "volume_ratio_20",
    ...
    def stage(result: EdgeScoutResult) -> str | None:
    ...
    rows = [(result, stage(result)) for result in results if result.tier is not None]
