#!/usr/bin/env python3
"""CLI for the read-only MKF red/blue cross candidate-source experiment."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NCN read-only MKF red/blue cross candidate-source experiment")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("MKF 候选源实验：开始读取本地日线并扫描红蓝线上穿20候选...", flush=True)
    ...
    print("selector=mkf_red_blue_cross20_under80_v1")
    ...
    if not rows:
        ...
        return 0
    print("\nMKF 红蓝线上穿20候选（只读研究，非买入建议）")
    print(f"{'#':>2}  {'code':<11} {'close':>8} {'amount(亿)':>10} {'momentum':>9} {'inter':>8} {'near':>8} reason")
