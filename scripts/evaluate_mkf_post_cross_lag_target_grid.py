#!/usr/bin/env python3
"""Evaluate MKF post-cross lag0..7 target/horizon capped-zero return grid."""
...
def _stock(task: tuple[str, dict[str, Any], str, str | None]) -> tuple[pd.DataFrame, dict[str, int]]:
    ...
    panel = build_mkf_post_cross_lag_target_grid_panel(path.stem, frame, config, start_date=start_date, end_date=end_date)
