#!/usr/bin/env python3
"""Evaluate full-universe rolling next-day validation candidates."""
...
def _evaluate_stock(task: tuple[str, dict[str, Any], str, str | None]) -> pd.DataFrame:
    path_text, config, start_date, end_date = task
    path = Path(path_text)
    frame = pd.read_parquet(path, columns=REQUIRED_COLUMNS)
    return build_nextday_panel(path.stem, frame, config, start_date=start_date, end_date=end_date)
