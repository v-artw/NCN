#!/usr/bin/env python3
"""Evaluate A-class T-open plus-3% five-day target-touch path quality."""
...
from ashare_edge_scout.research_a_class_target_touch import build_a_class_target_touch_panel, build_a_class_target_touch_report
from ashare_edge_scout.research_precision70 import PREFIXES

REQUIRED_COLUMNS = ["date", "open", "high", "low", "close", "preclose", "volume", "amount", "tradestatus", "isST", "turn"]
...
def _stock(task: tuple[str, dict[str, Any], str, str | None]) -> pd.DataFrame:
    ...
    return build_a_class_target_touch_panel(path.stem, frame, config, start_date=start_date, end_date=end_date)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    report = build_a_class_target_touch_report(
