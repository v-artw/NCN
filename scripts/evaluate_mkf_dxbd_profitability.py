#!/usr/bin/env python3
"""Evaluate isolated MKF-v3 then DXBD-control next-open T+1..T+10 returns."""
...
from ashare_edge_scout.pmkf_mkf.mkf_dxbd_profitability import build_mkf_dxbd_event_panel, build_mkf_dxbd_report
...
def _stock(task: tuple[str, dict[str, Any], str, str | None]) -> tuple[pd.DataFrame, dict[str, int]]:
    ...
    panel = build_mkf_dxbd_event_panel(path.stem, frame, config, start_date=start_date, end_date=end_date)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    report["event_diagnostics"] = dict(sorted(diagnostics.items()))
    report["processing_status_counts"] = dict(Counter("success" for _ in paths))
