#!/usr/bin/env python3
"""Evaluate full-sample 5/10-day intraday excursion classifications."""
...
from ashare_edge_scout.research_excursion_validation import DEFAULT_START_DATE, build_excursion_panel, build_excursion_report
...
def _stock(task: tuple[str, dict[str, Any], str, str | None]) -> pd.DataFrame:
    ...
    path = Path(path_text)
    return build_excursion_panel(path.stem, pd.read_parquet(path, columns=REQUIRED_COLUMNS), config, start_date=start_date, end_date=end_date)
