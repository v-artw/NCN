#!/usr/bin/env python3
"""Compare fair next-open MKF v3 and current production SMC T+1..T+10 returns."""
...
def _stock(task: tuple[str, dict[str, Any], str, str | None]) -> tuple[pd.DataFrame, pd.DataFrame]:
    path_text, config, start_date, end_date = task
    path = Path(path_text)
    frame = pd.read_parquet(path, columns=REQUIRED_COLUMNS)
    return build_stock_strategy_panels(path.stem, frame, config, start_date=start_date, end_date=end_date)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    mkf_panels: list[pd.DataFrame] = []
    smc_panels: list[pd.DataFrame] = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
        for completed, (mkf, smc) in enumerate(executor.map(_stock, tasks, chunksize=1), start=1):
            mkf_panels.append(mkf)
            smc_panels.append(smc)
    ...
    report = build_mkf_smc_report(
        mkf=pd.concat(mkf_panels, ignore_index=True),
        smc=pd.concat(smc_panels, ignore_index=True),
        code_list=code_list,
